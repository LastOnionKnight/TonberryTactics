...
recommendations. Up through v0.6.2 the optimizer was hardcoded to
GNB tank priority (Crit → DH → DET) for every job; the
"AST FALLBACK (GNB PRIORITY)" badge surfaced the embarrassing truth.
v0.6.3 routes recommendations through the new shared library
**GearGoblin.Core**, which carries per-job priority tables for all
21 combat jobs (plus BLU). The plugin and web now share a single
source of truth for which stats to recommend, sourced from public
guides at thebalanceffxiv.com.

This also restores the v0.5.5 lockstep commitment that v0.6.2 broke.
All three projects (Core, web, plugin) ship at v0.6.3 today.

### Added

- **ProjectReference to `GearGoblin.Core`** in `TonberryTactics.csproj`.
  Path: `..\..\GearGoblin-Core-v0.1\GearGoblin.Core\GearGoblin.Core.csproj` —
  assumes Core lives as a sibling-of-grandparent on disk. Adjust if
  your local clone uses a different layout. (See Core's README for
  the canonical directory structure.)

### Changed

- **`Services/PureMathOptimizer.cs` — full rewrite.** Now reads its
  priority list from `GearGoblin.Core.JobPriorities.For(jobAbbr)`
  rather than the hardcoded `GnbPriority` array. Materia name + stat
  value resolved via `Core.MateriaTiers.NameOf()` and `Core.MateriaTiers.SubstatValue()`
  so both halves agree on what "Tier XII" means.

- **`Pages/Index.razor`** — three things:
  - `CanonicalStatKey` is now a thin forwarder to
    `Core.StatNames.Canonical`. The matching rules (case
    insensitivity, whitespace stripping, "Direct Hit Rate" ↔ "DH"
    aliases) all live in Core so the plugin can share them.
  - Materia Advisor mode-tag text refreshed — no more "GNB priority
    [Crit › DH › Det]" with an apologetic "falling back to GNB
    priorities" warning. Now reads either "real AST priority from
    thebalanceffxiv.com" (table hit) or "falling back to GNB tank
    baseline (Core has no table for XYZ)" (genuinely unknown job).
  - `EmitterVersion` and `TtVersion` constants bumped 0.6.2 → 0.6.3.
    Version pill + footer copy bumped accordingly.

- **`TonberryTactics.csproj`** — version 0.6.2 → 0.6.3, Description
  refresh, new ProjectReference block (above).

### Pairing

Ships in lockstep with:

- **GearGoblin.Core v0.6.3** — new standalone repo, must be cloned
  to the expected path before the web can build. See the Core README
  for setup instructions.
- **GearGoblin plugin v0.6.3** — same ProjectReference added to its
  csproj. Plugin's existing job-aware MeldOptimizer continues to
  work unchanged; consuming Core's priority tables from the plugin
  side is an incremental migration tracked for v0.7.x.

### Verify after deploy

1. **Clone Core** to `D:\GearGoblin-Core-v0.1\GearGoblin.Core\` (or
   adjust the ProjectReference path).
2. **Build Core first** (`dotnet build --configuration Release`)
   so its .dll exists before the web tries to reference it.
3. **Build web** — the build gate in release.ps1 should succeed.
4. **Paste your AST export.** Materia Advisor badge should read
   "AST priority (via Core)" instead of "AST FALLBACK (GNB PRIORITY)".
   If your AST has empty meld slots, the recommended fills should
   reflect healer priority (Crit → DH → DET → SPS → PIE) rather
   than tank (Crit → DH → DET → SKS → TEN).

### Lockstep restored

v0.6.2 was a web-only patch with a CHANGELOG that explicitly carved
out an exception to the lockstep convention. v0.6.3 closes that
loop — all three projects ship same date, same version, same release
night, paired by intent rather than by coincidence. Going forward,
patch-level releases also bump in lockstep when there's anything
shared between halves; web-only or plugin-only patches that don't
touch shared logic can still be one-sided.

### Not changed

- Wire format (`GG-EXPORT:v1:`, `GG-PLAN:v1:`) untouched. v0.6.0+
  plugin exports parse identically.
- StatProfile role-aware Speed slot from v0.6.2 carries through.
- v0.6.1 stat-name canonicalization preserved (now via Core).
- All v0.6.0 design assets, fonts, CSS, layout unchanged.

---

## [0.6.2] — 2026-05-14  "Gear Division, role-aware"

**Headline:** Fixes the StatProfile row-5 hardcoding to Skill Speed.
Casters and healers (WHM, SCH, AST, SGE, BLM, SMN, RDM, PCT, BLU)
use Spell Speed in the same character-panel slot — v0.6.0 and v0.6.1
only knew about Skill Speed and any Spell Speed materia got silently
dropped on the display floor even with v0.6.1's CanonicalStatKey
fix correctly translating `"Spell Speed"` → `"SPS"`. The bucket
existed in the matching layer but not in the rendered grid.

Symptom: Brian's AST with three Quicktongue Materia XII (+54 SPS
each, +162 total) showed `+0 materia` on every Stat Profile row.
SKS/SPS in FFXIV share the same row position in the character
panel and never overlap on one job (a paladin never has Spell
Speed materia, a healer never has Skill Speed materia), so the
fix is to pick which name shows based on role rather than always
display SKS.

### Lockstep exception

Web-only ship. Plugin stays at v0.6.1 — `Services/PureMathOptimizer.cs`,
`Services/GearsetParser.cs`, and the emitted wire format are all
untouched. The bug was purely in `BuildStatCells()` on the display
side. Re-aligning the plugin to v0.6.2 with a no-op version bump
would be dishonest; v0.5.5's lockstep commitment was about
shipping new features in parallel, not about decorating bug-fix
patches with matching version numbers. Plugin gets its next bump
when there's plugin-side work to ship.

### Changed

- **`BuildStatCells()` in `Pages/Index.razor`** — role-aware speed
  slot selection. Computes `speedKey = isCasterOrHealer ? "SPS" : "SKS"`
  from `ParsedPayload.Character.JobAbbreviation`, picks the right
  display label out of a 7-entry label dictionary. Defensive: the
  totals dictionary initialises all seven possible buckets (CRT, DH,
  DET, SKS, SPS, TEN, PIE) so a cross-role outlier materia (rare but
  possible if someone melds a non-canonical stat) doesn't get
  silently dropped.

- **`IsCasterOrHealer(jobAbbr)`** (new private helper) —
  classifies the 9 caster/healer jobs (WHM, SCH, AST, SGE, BLM, SMN,
  RDM, PCT, BLU) into the SPS bucket; everything else uses SKS.
  Includes BLU defensively — Blue Mage uses Spell Speed though
  it's not in the design's main role rotation.

- **Version bumps:** `TonberryTactics.csproj` 0.6.1 → 0.6.2, plus
  `EmitterVersion` / `TtVersion` constants and the visible pills
  + footer in `Pages/Index.razor`.

### Not changed

- Wire format — `GG-EXPORT:v1:` and `GG-PLAN:v1:` schemas
  unchanged. v0.6.0 and v0.6.1 plugin exports still parse
  identically; the web just now buckets Spell Speed materia
  into a row that's actually visible.
- `Services/PureMathOptimizer.cs` — optimizer logic untouched.
  Per-job priorities still defer to GNB fallback for non-GNB
  jobs (v0.5.0 Core refactor on the queue).
- `CanonicalStatKey()` — the v0.6.1 helper carries through
  unchanged. It correctly mapped `"Spell Speed"` to `"SPS"`
  in v0.6.1 already; v0.6.2 just makes sure SPS has somewhere
  visible to live.

### Verify after deploy

Paste a healer/caster GG-EXPORT (your AST is perfect). Stat Profile
row 5 should now read **"Spell Speed"** with the actual materia
total (`+162` for three Quicktongue XII), cap bar partially filled.
Cap stays at 3,140 — full audit hits v0.6.3+/Core refactor for the
real tier breakpoints.

---

## [0.6.1] — 2026-05-13  "Gear Division, hotfix"

**Headline:** Fixes the stat-name string mismatch that made every Stat
Profile row read `+0 materia` and every gear-grid materia dot render
without a per-stat colour class, even on a fully-melded gearset.

### The bug

The plugin's `GearsetExporter` writes the in-game **BaseParam display
names** verbatim into `ExportMateriaV1.StatName` — `"Critical Hit"`,
`"Direct Hit Rate"`, `"Determination"`, `"Tenacity"`, `"Skill Speed"`,
`"Spell Speed"`, `"Piety"`. The v0.6.0 web only knew the abbreviated
forms — `"CRT"`, `"DH"`, `"DET"`, `"TEN"`, `"SKS"`, `"SPS"`, `"PIE"`.
`BuildStatCells()` and `MateriaStatClass()` both keyed on a raw
`statName.ToUpper()` against those abbreviations, so:

- Every materia missed its `totals` dictionary lookup → all six
  rows in the Stat Profile rendered `+0 materia / 0% fill` even
  for fully-melded gear with 20 melds parsed.
- Every materia dot in the gear grid resolved to an empty CSS
  class → all dots rendered without their per-stat colour (no
  blue for Direct Hit, no red for Crit, etc.).

The "PARSED · 20 MELDS" badge correctly reported 20 because that count
comes from `Equipped[].Materia.Count` summed, which never touched
`StatName`. Symptom was subtle: the page parsed the export fine, said
"yes I saw 20 melds," then mis-attributed every one of them.

### The fix

New `CanonicalStatKey(string? statName)` helper in `Pages/Index.razor`
that accepts both abbreviated and full BaseParam forms (case-insensitive,
whitespace-stripped):
...
- **`BuildStatCells()`** — uses `CanonicalStatKey(m.StatName)` to bucket
  each materia's `StatValue` into the right Stat Profile row.
- **`MateriaStatClass(statName)`** — switch now matches against
  `CanonicalStatKey(statName)`, so gear-grid materia dots pick up the
  right per-stat CSS colour class regardless of which name form the
  plugin emitted.

### Pairing

Pairs with **plugin v0.6.1** which separately fixes the v0.4.7-scaffold
hardcoded-empty-clipboard bug in `Services/GearsetImporter.cs::ImportFromClipboard`.
Same release night, same version number, two complementary hotfixes on
opposite sides of the round-trip. v0.5.5's lockstep commitment holds —
both halves still at the same version.

### Files touched

- `Pages/Index.razor` — `CanonicalStatKey` added, `MateriaStatClass`
  and `BuildStatCells` rewired through it, version pill + footer +
  `EmitterVersion` + `TtVersion` constants bumped to 0.6.1
- `TonberryTactics.csproj` — version 0.6.0 → 0.6.1, hotfix description
- `CHANGELOG.md` — this entry

### Not changed

- Wire format (`GG-EXPORT:v1:`, `GG-PLAN:v1:`) untouched. Any v0.6.0
  plugin export still parses the same; the web just now buckets the
  materia correctly. Round-trip stays compatible.
- `Services/PureMathOptimizer.cs` — optimizer logic untouched.
- All v0.6.0 design assets, fonts, CSS, layout — untouched.
